...
airp <- read.csv("KGON-Aug2016.csv", stringsAsFactors=FALSE)
...
p <- p + 
  ...
  # December - 31
  geom_vline(xintercept = 184, colour = "wheat4", linetype=3, size=.5) +
  # January - 31
...
title <- paste0(town, "'s weather from August 2015 through July 2016")
...
p <- p +
  ...
  annotate("text", x = 100, y = 5, label = "2015 RANGE", size=3.5, colour="gray30") +
  annotate("text", x = 40, y = 13, label = "RECORD HIGH", size=3, colour="gray30") +
  annotate("text", x = 41, y = -3, label = "RECORD LOW", size=3, colour="gray30") +
...
filenamesvg <- paste0(town, "-Aug2016.svg")
filenamepng <- paste0(town, "-Aug2016.png")
